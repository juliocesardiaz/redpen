 ...
 *
 * Load order: core → import → comments → tags → main. Files share state by
 * mutating window.Redpen.state properties (never module-scope `let`s, which
 * would not cross the <script> boundary). Cross-file calls go through `R.`;
 * within-file calls stay bare.
 */
